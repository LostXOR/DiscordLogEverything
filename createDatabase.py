...
def createDatabase(databasePath):
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS Interaction(uuid PRIMARY KEY, timestamp, deleted, event_uuid, id, message_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS StageInstance(uuid PRIMARY KEY, timestamp, deleted, event_uuid, id, channel_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS Achievement(uuid PRIMARY KEY, timestamp, deleted, event_uuid, id, name)")
    ...
    # One column for each message ID, up to a maximum of 100 for a bulk delete (allows querying for message IDs)
    messageColumnsList = ", ".join([f"message_id_{i}" for i in range(100)])
    cursor.execute(f"CREATE TABLE IF NOT EXISTS EventBulkMessageDelete(uuid PRIMARY KEY, timestamp, {messageColumnsList})")
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS EventRelationshipAdd(uuid PRIMARY KEY, timestamp, user_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventRelationshipRemove(uuid PRIMARY KEY, timestamp, user_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventRelationshipUpdate(uuid PRIMARY KEY, timestamp, user_id)")
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS EventInviteCreate(uuid PRIMARY KEY, timestamp, invite_id, guild_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventInviteDelete(uuid PRIMARY KEY, timestamp, invite_id, guild_id)")
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS EventInteraction(uuid PRIMARY KEY, timestamp, interaction_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventInteractionFinish(uuid PRIMARY KEY, timestamp, interaction_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventStageInstanceCreate(uuid PRIMARY KEY, timestamp, stage_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventStageInstanceDelete(uuid PRIMARY KEY, timestamp, stage_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventStageInstanceUpdate(uuid PRIMARY KEY, timestamp, stage_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventPremiumGuildSubscriptionSlotCreate(uuid PRIMARY KEY, timestamp, slot_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventPremiumGuildSubscriptionSlotUpdate(uuid PRIMARY KEY, timestamp, slot_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventAchievementUpdate(uuid PRIMARY KEY, timestamp, achievement_id, percent_complete)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventEntitlementCreate(uuid PRIMARY KEY, timestamp, entitlement_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventEntitlementUpdate(uuid PRIMARY KEY, timestamp, entitlement_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventEntitlementDelete(uuid PRIMARY KEY, timestamp, entitlement_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventModal(uuid PRIMARY KEY, timestamp, modal_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventIntegrationCreate(uuid PRIMARY KEY, timestamp, integration_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventIntegrationUpdate(uuid PRIMARY KEY, timestamp, integration_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventIntegrationDelete(uuid PRIMARY KEY, timestamp, integration_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventGuildIntegrationsUpdate(uuid PRIMARY KEY, timestamp, guild_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventWebhooksUpdate(uuid PRIMARY KEY, timestamp, channel_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventVoiceStateUpdate(uuid PRIMARY KEY, timestamp, user_id)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventBillingPopupBridgeCallback(uuid PRIMARY KEY, timestamp, payment_source_type, path, query, state)")
    cursor.execute("CREATE TABLE IF NOT EXISTS EventGuildStickersUpdate(uuid PRIMARY KEY, timestamp, guild_id)")
